...
def load_data():
    ...
    train_ds = load_dataset("wav2gloss/fieldwork", split="train").select_columns(["transcription", "language", "audio"])
    train_df = train_ds.to_pandas()
    train_df = train_df[train_df["language"] == config["language"]]
    train_df = train_df[train_df["transcription"] != ""]
    train_df["ipa_transcription"] = train_df["audio"].map(get_ipa)
...
def save_preds(preds: np.ndarray | tuple[np.ndarray], metrics: dict[str, float] | None,
    ...
    with open("out/" + config["language"] + "_audio_based_ipa_preds.txt", "w", encoding="utf-8") as g:
        g.write(str(metrics) + "\n\n")
